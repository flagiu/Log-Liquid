...
#define L 43.32963             // box size
#define DN 0.5           // q-vector mesh size in QVECTORS/ files

#define MAXQARR 300     // max n. of qvectors with same q
#define MAXCOUNT 1000    // max n. of extractions before quitting

#ifndef MESHFACT
#define MESHFACT 1    //(positive integer) increases the mesh size
#endif

#define Dq (2*M_PI/L)// * MESHFACT)
#define dq (DN*Dq)
...
int main(int argc, char** argv){
  ...
  if(argc!=2) {
    fprintf(stderr,
	    "\n Usage: %s [wavenumber]\n"
	    "\n Generate random wavenumbers (max %d) in the spherical shell (nmin, nmax].\n"
	    " - n is an integer in mesh units; mesh is 2pi/L*(DN=%.1f)*(MESHFACT=%d))\n"
	    " - nmax,nmin = DN*MESHFACT*(n+-.5)\n\n", argv[0], MAXQARR, DN, MESHFACT);
    exit(1);
  }
  n=atoi(argv[1]);
  n1=DN*MESHFACT*((double)n-0.5);
  n2=DN*MESHFACT*((double)n+0.5);
  srand48(time(NULL));

  fprintf(stdout, "\nWill generate integer wavenumbers with norm in (%.2f,%.2f]\n\n",n1,n2);
  ...
  for(i=0;i<MAXQARR && count<MAXCOUNT;i++){
	...
	if(equivalent) fprintf(stderr, "%d %d %d === %d %d %d\n",
  ...
  if(n>300 || MESHFACT>1) {
    FILE *fp;
    char file[50]={0};
    if(MESHFACT==1) sprintf(file, "QVECTORS/qvector.%03d", n);
    else sprintf(file, "QVECTORS%03d/qvector.%03d", MESHFACT, n);
