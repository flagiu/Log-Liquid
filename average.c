...
void printaverage(double R0[2], double dt, double dt2, double R, double R2, double I, double I2, int N) {
  double mt,st,mR,mI,sR,sI,phi,sphi, M=(double)N;
  mt=dt/M;
  ...
  if (fabs(mt)<EPS && st<EPS) {
    R0[0]=mR;
    R0[1]=sR;
  }
  phi=mR/R0[0];
  sphi = phi*sqrt( (sR/mR)*(sR/mR) + (R0[1]/R0[0])*(R0[1]/R0[0]) );
  printf("%18le %18le %18le %18le %18le %18le %18le %18le %d\n",mt,st, mR,sR, mI,sI, phi, sphi, N);
  if(mt==9.0584) fprintf(stderr,"%f %f %d\n",dt,dt2,N);
...
int main(int argc, char **argv) {
  ...
  double R0[2]; //mean and std.dev of F(q,t=0) -> useful for phi(q,t)
  FILE *fp;
  ...
  if (fp != NULL) {
	...
	printaverage(R0,dt,dt2,R,R2,I,I2,N);
