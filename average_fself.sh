...
gcc -Wall -O3 average_fself.c -lm -o average.x

for ((i=$qmin;i<=$qmax;i+=dq)); do
    printf -v logdata "fself_q%03d_m%03d_nat%d-log.dat" $i $M $NAT
    printf -v lindata "fself_q%03d_m%03d_nat%d-lin.dat" $i $M $NAT
    printf -v infile "fself_q%03d_m%03d_nat%d.dat" $i $M $NAT
    printf -v outfile "fself_ave_q%03d_m%03d_nat%d.dat" $i $M $NAT
    printf -v phifile "Fs_q%03d_m%03d_nat%d.dat" $i $M $NAT
    printf "q%03d ...\n" $i
