...
int main(int argc, char **argv) {
  ...
  double ts[MAXCNF], Dt, *e0, *e1, rho0[2], rhot[2], fqt[2];
  char logfile[40], Dtmodestr[4], outfile[40], qvecfile[100];
  FILE *fp;
  ...
  /////////// !!!! ////////
  // this is the maximum absolute wavenumber along a single direction
  // among all the qvector.xxx files that will be loaded, i.e. the maximum
  // absolute wavenumber in the QVECTORS(M)/qvector.(nmax2) file
  qnmax=floor( DN*MESHFACT*(nmax2+0.5) ); //max from theory
  
  if(MESHFACT>1) sprintf(qvecfile, "QVECTORS%03d/qvector.%03d",MESHFACT,nmax2);
  else sprintf(qvecfile, "QVECTORS/qvector.%03d",nmax2);
  getMinMaxWavenumber(qvecfile, minmax); //min,max from brute-force reading
  
  if(qnmax<minmax[1]) {
    fprintf(stderr, "Error: max absolute wavenumber from theory is lower than the real one: respectively %d and %d\n\n", qnmax, minmax[1]);
    exit(1);
  }
  ////////// !!!! /////////

  // experimental criterion for preallocating or not the exp(i*q*r) values
  onfly=(2*nmax2*MESHFACT>MAXNMAX);
  if(onfly)
    {
  ...
  //**** CHECKPOINT LOADING: restart from when it was interrupted  ********
  // following the order of the main cycles
  fprintf(stderr,"\nLoading checkpoints...\n");
  n=0;
  for(i=0;i<Nnmax;i++) {
    nmax=nmax1+i*dnmax;
    sprintf(logfile, "q%03d_m%03d_nat%d-%s.logc", nmax,MESHFACT,NAT,Dtmodestr);
    if(MESHFACT>1) sprintf(qvecfile, "QVECTORS%03d/qvector.%03d",MESHFACT,nmax);
    else sprintf(qvecfile, "QVECTORS/qvector.%03d",nmax);
  ...
  fprintf(stderr,"...done.\nEarliest starting point is t0=%d, Dt=%d for q=%d\n\n",t0restart,Dtrestart,(nmax1+n*dnmax));
  //*************************************************************

  //***** MAIN NESTED CYCLES *******
  // 1) CYCLE over starting time t0 *****************************
  ...
  for (jj=t0restart; jj<lenT0cycle; jj++) {
    ...
    fprintf(stderr,"\n");
    // 2) CYCLE over time difference Dt ******************************
    ...
    for (ii=Dtrestart; ii<lenDTcycle; ii++) {
      ...
      fprintf(stderr," Computing ISF at t0=%f (%d/%d); Dt=%f (%d/%d) for all q's... ",ts[j],jj+1,lenT0cycle,Dt,ii+1,lenDTcycle);
      // 3) CYCLE over wavenumber files (numbered by nmax) ****************
      for(n=0;n<Nnmax;n++) {
	...
	nmax=nmax1+n*dnmax;
	sprintf(logfile,     "q%03d_m%03d_nat%d-%s.logc", nmax,MESHFACT,NAT,Dtmodestr);
	sprintf(outfile, "fqt_q%03d_m%03d_nat%d-%s.dat",  nmax,MESHFACT,NAT,Dtmodestr);
	sprintf(qvecfile, "QVECTORS");
	if(MESHFACT>1) sprintf(qvecfile, "QVECTORS%03d/qvector.%03d",MESHFACT,nmax);
	else sprintf(qvecfile, "QVECTORS/qvector.%03d",nmax);
	Nqvec = getQvectors(qvecfile, qarr);
	fp=fopen(outfile, "a");
	// 4) CYCLE over each wavenumber vector inside a given file ***********
	for (l=0; l<Nqvec; l++) {
	  ...
	  calcRHORHO(fqt, e0,e1, qarr[l], rho0,rhot, printexp, onfly);
	  savelog(logfile, jj,ii,l);
	  fprintf(fp, "%d %d %18e %d %d %d %18e %18e\n",
		  j,i,Dt,qarr[l][0],qarr[l][1],qarr[l][2],fqt[0],fqt[1]
		  );
	...
	} // 4) completed all wavenumbers inside a given file
	fflush(fp);
	fclose(fp);
      } // 3) completed all wavenumber files at a given Dt
    } // 2) completed all Dt at a given t0
    fprintf(stderr,"done\n");
  } // 1) completed all t0
...
void printErr(char *myname) {
  fprintf(stderr,
	  "\nNormal Usage: %s <Dt-mode> <q-min> [q-max=q-min] [dq=1]\n\nQuesto programma calcola la Intermediate Scattering Function F(q,Dt) a partire dai frame corrispondenti al pattern ../Cnf*-*. Per tutti i vettori di modulo q (vedi cartella QVECTORS), per q compreso in [qmin,qmin+dq,...,qmax]. La F((qx,qy,qz),t0,Dt) viene appesa al file FQTC/fqt_qXXX.dat. Questi dati dovranno essere mediati esternamente su t0 e sul vettore q.\n Dt-mode\t %d (log) per dt piccoli; %d (lin) per dt grandi.\n"
